...
- `/etc/vault-approle.env` — a scoped Vault AppRole (policy `service-host`: read on `kv/data/aws/iam_access_keys/*`, `kv/data/splunk/*`, `kv/data/proxmox/*`, `kv/data/jenkins/*`, `kv/data/ssh/*`, `kv/data/theia/*`, `kv/data/opnsense/*`, `kv/data/github/*`, read/write on `kv/data/ssl_certs/*` and `kv/data/k8clusters/*`) for `service`'s own ongoing use. The Vault *root* token is not stored long-term on `service` — it's only used interactively to bootstrap things like this. (`k8clusters` needs write too, not just read: a fresh control-plane node generates and stores its own join token/cert hash there during `kubeadm init` -- found via the first real rebuild-fleet.sh run failing to store it.)
