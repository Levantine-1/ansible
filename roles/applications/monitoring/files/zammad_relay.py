...
# Same Vault-sourced id the incident agent uses for _assign_to_human() (see
# roles/applications/incident-agent). 0 means "no human-admin account
# configured" -- every resolved alert just closes as it always did.
ZAMMAD_HUMAN_ADMIN_USER_ID = int(os.environ.get("ZAMMAD_HUMAN_ADMIN_USER_ID", "0"))
...
def handle_alert(alert):
    ...
    if status == "firing":
        ...
        ticket = zammad_request(
            ...
            "/api/v1/tickets",
            {
                ...
                "article": {
                    ...
                    "body": f"*Comment generated by: script: zammad_relay (ticket created from alert)*\n\n{summary}",
                    "type": "note",
                    "internal": False,
                },
            },
        ...
        # Skip the auto-close if a human has explicitly taken this ticket
        # (owner_id == the human-admin account) -- that's exactly the state
        # the incident agent's _assign_to_human() puts a ticket into when
        # Claude determines the only real fix is a config change, and the
        # whole point of that hand-off is that the ticket stays open and
        # visible until the human deals with it. Without this check, the
        # very next time Alertmanager reports the alert cleared (which, for
        # a flapping alert, can be soon) would auto-close it regardless,
        # quietly undoing the hand-off and reopening the door to a fresh
        # ticket -- and a fresh, fully-paid re-triage -- on the next re-fire.
        # Confirmed live and costly without this (2026-09-03): a single
        # mis-tuned alert rule produced 7 separate tickets in 2 days this
        # way, each independently re-investigated at full cost.
        if ZAMMAD_HUMAN_ADMIN_USER_ID and existing.get("owner_id") == ZAMMAD_HUMAN_ADMIN_USER_ID:
            zammad_request(
                "POST",
                f"/api/v1/ticket_articles",
                {
                    "ticket_id": existing["id"],
                    "subject": "Alert cleared, left open",
                    "body": (
                        "*Comment generated by: script: zammad_relay (alert resolved by Alertmanager)*"
                        f"\n\nAlertmanager reports this alert as resolved: {summary}\n\n"
                        "Left open, not auto-closed: this ticket is assigned to a human for a "
                        "config-change review. Close it once that's done."
                    ),
                    "type": "note",
                    "internal": False,
                },
        ...
        zammad_request(
            ...
            f"/api/v1/tickets/{existing['id']}",
            {
                ...
                "article": {
                    ...
                    "body": f"*Comment generated by: script: zammad_relay (alert resolved by Alertmanager)*\n\nAlertmanager reports this alert as resolved: {summary}",
                    "type": "note",
                    "internal": False,
                },
            },
